...
BlockchainMonitor.prototype.start = function(opts, cb) {
  ...
  async.parallel([
    ...
    function(done) {
      ...
      _.forEach(coinNetworkPairs, function(pair) {
        var explorer;
    ...
    // TODO Assets are obtained once on startup; have Keoken Explorer notify KWS when an asset is added,
    // in order to update this list without polling
    function(done) {
      var keokenExplorer = opts.keokenExplorer || new KeokenExplorer(opts.keokenExplorerOpts);
      keokenExplorer.getAssets(function(err, assets){
        if (err) {
          log.error('Could not access Keoken explorer for listing assets');
          done();
        } else {
          self.assets = _.keyBy(assets, 'asset_id');
          done();
        }
      })
    },
...
BlockchainMonitor.prototype._handleThirdPartyBroadcasts = function(data, processIt) {
  ...
  self.storage.fetchTxByHash(data.txid, function(err, txp) {
    ...
    self.storage.fetchNotifications(walletId, null, fromTs, function(err, notifications) {
      if (err) {
        log.error("Could not fetch notifications from the db: " + err);
        return;
      }
      var alreadyNotified = _.some(notifications, function(n) {
        return n.type == 'NewOutgoingTxByThirdParty' && n.data && n.data.txid == data.txid;
      });
      if (alreadyNotified) {
        log.info('The outgoing tx ' + data.txid + ' was already notified');
        return;
      }
      self.storage.softResetTxHistoryCache(walletId, null, function() {
...
BlockchainMonitor.prototype._handleIncomingPayments = function(coin, network, data) {
  ...
  var outs = _.compact(_.map(data.vout, _.bind(_compactVout, null, _, coin) ));
  ...
  async.each(outs, function(out, next) {
    ...
    self.storage.fetchAddressByCoin(coin, out.address, function(err, address) {
      ...
      self.storage.fetchNotifications(walletId, null, fromTs, function(err, notifications) {
        ...
        var assetId = Defaults.KEOS_ASSET_ID;
        if(out.keoScript) {
          assetId = KeokenScriptParser.parseAssetId(out.keoScript);
          var assetAcronym = self.assets[assetId].asset_name.substring(0, 3).toUpperCase();
          txData.keoAmount = KeokenScriptParser.parseAmount(out.keoScript) + ' ' + assetAcronym;
        } else {
          txData.keoAmount = '0 KEO (' + txData.amount/100000000 + ' BCH)';
        }
        ...
        self.storage.softResetTxHistoryCache(walletId, assetId, function() {
...
BlockchainMonitor.prototype._notifyNewBlock = function(coin, network, hash) {
  var self = this;
  var fromTs = Date.now() - 24 * 3600 * 1000;
  // use network name as wallet id for global notifications
  self.storage.fetchNotifications(network, null, fromTs, function(err, notifications) {
    if (err) {
      log.error("Could not fetch notifications from the db: " + err);
      return;
    }
    var alreadyNotified = _.some(notifications, function(n) {
      return n.type == 'NewBlock' && n.data && n.data.coin == coin && n.data.network == network && n.data.hash == hash;
    });
    if (alreadyNotified) {
      log.info('The new block ' + hash + ' for ' + coin + ' ' + network + ' was already notified');
      return;
    }
...
BlockchainMonitor.prototype._handleTxConfirmations = function(coin, network, hash) {
  ...
  function processTriggeredSubs(subs, cb) {
    async.each(subs, function(sub) {
      var fromTs = Date.now() - 24 * 3600 * 1000;
      self.storage.fetchNotifications(sub.walletId, null, fromTs, function(err, notifications) {
        if (err){
          log.error("Could not fetch notifications: " + err);
          return next(err)
        };
        var alreadyNotified = _.some(notifications, function(n) {
          return n.type == 'TxConfirmation' && n.data && n.data.txid == sub.txid;
        });
        if (alreadyNotified) {
          log.info('Confirmation for tx ' + sub.txid + ' was already notified');
          return next();
        }
        // Not notified before

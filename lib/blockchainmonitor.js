...
BlockchainMonitor.prototype._handleThirdPartyBroadcasts = function(data, processIt) {
  var self = this;
  if (!data || !data.txid){
    log.warn("Undefined transaction, or lacking txid: " + JSON.stringify(data));
    return;
  }
  ...
  self.storage.fetchTxByHash(data.txid, function(err, txp) {
    ...
    if (!txp || txp.status != 'accepted'){
      log.debug('Transaction ' + data.txid + ' not stored or not accepted, skipping; txp: ' + JSON.stringify(txp));
      return;
    }
...
BlockchainMonitor.prototype._handleIncomingPayments = function(coin, network, data) {
  var self = this;
  if (!data || !data.vout) {
    log.warn('Undefined payment, or lacking vout: ' + JSON.stringify(data));
    return;
  }
...
BlockchainMonitor.prototype._notifyNewBlock = function(coin, network, newBlockMessage) {
  var self = this;

  log.info('New ' + network + ' block: ' + newBlockMessage.blockhash);
  var notification = Notification.create({
    type: 'NewBlock',
    walletId: network, // use network name as wallet id for global notifications
    data: {
      hash: newBlockMessage.blockhash,
      coin: coin,
      network: network,
    },
  });
...
BlockchainMonitor.prototype._handleTxConfirmations = function(coin, network, newBlockMessage) {
  var self = this;
  ...
  explorer.getTxidsInBlock(newBlockMessage.blockhash, function(err, txids) {
    if (err) {
      log.error('Could not fetch txids from block ' + newBlockMessage.blockhash, err);
      return;
    }
...
BlockchainMonitor.prototype._handleNewBlock = function(coin, network, newBlockMessage) {
  this._notifyNewBlock(coin, network, newBlockMessage);
  this._handleTxConfirmations(coin, network, newBlockMessage);
};

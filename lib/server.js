...
var KeokenExplorer = require('./blockchainexplorers/keoken');
var KeokenScriptParser = require('./keoken/scriptparser');
...
function WalletService() {
  ...
  this.keokenExplorer = keokenExplorer;
  this.keokenExplorerOpts = keokenExplorerOpts;
...
WalletService.initialize = function(opts, cb) {
  ...
  keokenExplorer = opts.keokenExplorer;
  keokenExplorerOpts = opts.keokenExplorerOpts;
 ...
 * @param {string} opts.singleAddress[=true] - Keoken needs only one address.
...
WalletService.prototype.getWalletFromIdentifier = function(opts, cb) {
  ...
  async.parallel([
    ...
    _.forEach(_.values(Constants.COINS), function(coin) {
      _.forEach(_.values(Constants.NETWORKS), function(network) {
...
WalletService.prototype._getKeokenExplorer = function() {
  var opts = this.keokenExplorerOpts;

  if (this.keokenExplorer) return this.keokenExplorer;
  var bc;
  try {
    bc = new KeokenExplorer(opts);
  } catch (ex) {
    this.logw('Could not instantiate blockchain explorer', ex);
  }
  return bc;
...
WalletService.prototype._hasKeoBalance = function(keoOpts, cb) {
  var self = this;
  var bc = self._getKeokenExplorer();

  if (!bc) {
    return cb(new Error('Could not get keoken explorer instance'), false);
  }

  var keoOptions = keoOpts;

  bc.getAssetsByAddress(keoOptions.Address, function( error, body ) {
    ...
    body.forEach( function (element) {
      if (!found) {
        if (element.asset_id == keoOptions.Id) {
          // The wallet has tokens
          found = true;
          amount = element.amount;
        }
      }
    });
    if (keoOptions.Amount <= amount) {
      return cb(null, true);
    } else {
...
WalletService.prototype._validateKeokenScript = function (output) {
  
  // TODO (guille): this should be global
  const keo_prefix = "6a0400004b50"; // op_return + keoken prefix
  const version = "100000"; // 10hex = 16 bytes lenght + version (0)

  var script = output.script;
  // var send_token = "0001"; 
  // var create_asset = "0000";

  if (keo_prefix == script.substring(0, 12) && version == script.substring(12, 18)) {
    // var type = script.substring(18, 22);
    // if (type == send_token) {
    //   var asset_id = script.substring(22, 30);
    //   var amount = script.substring(30);
    // }
    return true;
  }
  return false;
...
WalletService.prototype.getKeoBalance = function(wallet, cb) {

  var self = this;

  function getSingleAddress(wallet, cb) {
  ...
  getSingleAddress(wallet, function (err, address) {
    if (err) return cb(err, 0);
    var keokenAddress = address.address;

    bc.getAssetsByAddress(keokenAddress, function( error, body ) {
      if(error != null) {
        return cb(error, 0);
      }
      var found = false;
      ...
      body.forEach( function (element) {
        if (!found) {
          // TODO (guille): Hardcoded 1 for keos
          if (element.asset_id == 1) {
            // The wallet has tokens
            found = true;
            amount = element.amount;
          }
...
WalletService.prototype.getKeoAssets = function(cb) {
  var self = this;
  var keoExplorer = self._getKeokenExplorer();
  if (!keoExplorer) {
    return cb(new Error('Could not get keoken explorer instance'), false);
  }
  keoExplorer.getAssets(function(err, body){
    if (err) return cb(err, false);
    return cb(null, body);
  });
}

WalletService.prototype.storeAssetBinding = function(walletId, assetId, cb) {
  var self = this;
  self.storage.storeAssetBinding(walletId, assetId, cb);
}

WalletService.prototype.fetchWalletAssets = function(walletId, cb) {
  var self = this;
  self.storage.fetchWalletAssets(walletId, cb);
}
  ...
  async.series([
    ...
    function(next) {
      self.getPendingTxs({}, function(err, txps) {
        ...
        _.forEach(lockedInputs, function(input) {
          if (utxoIndex[input]) {
    ...
    function(next) {
      ...
      self.storage.fetchBroadcastedTxs(self.walletId, {
        ...
        _.forEach(spentInputs, function(input) {
          if (utxoIndex[input]) {
    ...
    function(next) {
      ...
      _.forEach(allUtxos, function(utxo) {
  ...
  self._runLocked(cb, function(cb) {
    checkBalanceCache(function(err, cache) {
      ...
      self._getUtxosForCurrentWallet({
        ...
        var byAddress = {};
        _.forEach(_.keyBy(_.sortBy(utxos, 'address'), 'address'), function(value, key) {
          byAddress[key] = {
            address: key,
            path: value.path,
            amount: 0,
...
WalletService.prototype._sampleFeeLevels = function(coin, network, points, cb) {
  ...
  bc.estimateFee(points, function(err, result) {
    ...
    var failed = [];
    var levels = Utils.zipObject(_.map(points, function(p) {
...
WalletService.prototype._checkTx = function(txp) {
  ...
  var serializationOpts = {
    ...
    // There is a bug in bitcore-lib-cash. If the op_return output has more than 1 push_data, it forces the output to send at least dust.
    // The op_return outputs should always send 0 satoshis because they can not be used by an input.
    // To allow the BWS to validate a keoken transaction the disableBustOutputs flag is set.
...
WalletService.prototype._selectTxInputs = function(txp, utxosToExclude, cb) {
  ...
  function select(utxos, cb) {
    ...
    _.forEach(smallInputs, function(input, i) {
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    // is_script variable adds support for keoken scripts as outputs
    var is_script = false;

    if (!checkRequired(output, ['toAddress', 'amount'])) {
      if (!checkRequired(output, ['script', 'amount'])){
    ...
    if (!is_script){
      ...
      // Validate the keoken format
      if (this._validateKeokenScript(output) == false) {
        return Errors.INVALID_SCRIPT;
      }
    }
...
WalletService.prototype.createTx = function(opts, cb) {
  ...
  self._runLocked(cb, function(cb) {
    var txp, changeAddress, feePerKb, keokenAddress;
    self.getWallet({}, function(err, wallet) {
      if (err) return cb(err);
      if (!wallet.isComplete()) return cb(Errors.WALLET_NOT_COMPLETE);
      ...
      checkTxpAlreadyExists(opts.txProposalId, function(err, txp) {
        ...
        async.series([
          ...
          function(next) {
            getSingleAddress(wallet, function (err, address) {
              if (err) return next(err);
              keokenAddress = address.address;
              next();
            });
          }, function(next) {
            var keoOpts = {
              Address: keokenAddress,
              Id: opts.keoken.keoken_id,
              Amount: opts.keoken.keoken_amount
            }
            self._hasKeoBalance(keoOpts, function(err, hasBalance) {
              if (err) return next(err);
              if (!hasBalance) return next(Errors.NOT_ENOUGH_KEOKEN_BALANCE);
              next();
            });
          },
          function(next) {
...
WalletService.prototype._processBroadcast = function(txp, opts, cb) {
  ...
  $.checkState(txp.txid);
  opts = opts || {};
  opts.assetId = opts.assetId || Defaults.KEOS_ASSET_ID;
  ...
  self.storage.storeTx(self.walletId, txp, function(err) {
    ...
    self.storage.softResetTxHistoryCache(self.walletId, opts.assetId, function() {
      return cb(err, txp);
    });
  });
};
...
WalletService.prototype._normalizeTxHistory = function(txs) {
  ...
  return _.map([].concat(txs), function(tx) {
    ...
    var outputs = _.map(tx.vout, function(item) {
      ...
      //Zero-value output: contains Keoken script
      if(Math.abs(normalizedOutput.amount - 0) < Number.EPSILON) {
        normalizedOutput.script = item.scriptPubKey.hex;
        if (!tx.assetId) {
          tx.assetId = KeokenScriptParser.parseAssetId(item.scriptPubKey.hex);
        }
      }
 ...
 * @param {Number} opts.filterByAssetId
 * @returns {TxProposal[]} Transaction proposals, newer first
 */
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  opts.filterByAssetId = opts.filterByAssetId || Defaults.KEOS_ASSET_ID;
    ...
    return _.map(txs, function(tx) {
      ...
      if (opts.includeExtendedInfo) {
        ...
        newTx.outputs = _.map(outputs, function(output) {
          return _.pick(output, ['address', 'amount', 'isMine', 'script']);
        });
      } else {
      ...
      if (proposal) {
        ...
        _.forEach(newTx.outputs, function(output) {
          var query = {
  ...
  function getNormalizedTxs(wallet, addresses, from, to, assetId, cb) {
    ...
    async.series([
      ...
      function(next) {
        ...
        self.storage.getTxHistoryCache(self.walletId, from, to, assetId, function(err, res) {
          if (err) return next(err);
      ...
      function(next) {
        ...
        bc.getTransactions(addressStrs, from, to, function(err, rawTxs, total) {
          ...
          txs = _.filter(txs, tx => tx.assetId && tx.assetId === opts.filterByAssetId);
          totalItems = total;
          return next();
        }, self.walletId);
      },
      function(next) {
        ...
        self.storage.storeTxHistoryCache(self.walletId, totalItems, fwdIndex, txsToCache, opts.filterByAssetId, next);
      },
      function(next) {
        if (!useCache || !fromCache) return next();
        if (!txs) return next();
    ...
    self.getFeeLevels({
      ...
      if (err) {
        ...
        if (!level || !level.nbBlocks) {
          ...
          _.forEach(unconfirmed, function(tx) {
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self.storage.fetchAddresses(self.walletId, function(err, addresses) {
      ...
      async.waterfall([
        ...
        function(next) {
          getNormalizedTxs(wallet, addresses, from, to, opts.filterByAssetId, next);
        },
        function(txs, next) {
          // Fetch all proposals in [t - 7 days, t + 1 day]
          var minTs = txs.items.length === 0? NaN : _.minBy(txs.items, 'time').time - 7 * 24 * 3600;
          var maxTs = txs.items.length === 0? NaN : _.maxBy(txs.items, 'time').time + 1 * 24 * 3600;
 ...
 * @param {Integer} opts.assetId (defaults to KEO asset id) Scan for txs from a specific asset
 */
WalletService.prototype.scan = function(opts, cb) {
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.clearTxHistoryCache(self.walletId, opts.assetId, function() {
        self.storage.storeWallet(wallet, function(err) {
          if (err) return cb(err);

          var derivators = [];
          _.forEach([false, true], function(isChange) {
            derivators.push({
            ...
            if (opts.includeCopayerBranches) {
              _.forEach(wallet.copayers, function(copayer) {

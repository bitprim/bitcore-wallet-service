...
WalletService.prototype._hasKeoBalance = function(opts, cb) {
  var self = this;
  ...
  bc.getAssetsByAddress(opts.outputs[0].toAddress, function( error, body ) {
    ...
    body.forEach(function (element) {
      if (element.asset_id == opts.keoken.keoken_id) {
        // The wallet has tokens
        if (element.amount >= opts.keoken.keoken_amount) {
          // The wallet has enough keoken balance
          return cb(null, true);
        } else {
          // The wallet has NOT enough keoken balance
          return cb(null, false);      
        }
      }
    });
    // The wallet has no tokens = keoken_id -> balance = 0
    return cb(null, false);
    ...
    self.getWallet({}, function(err, wallet) {
      ...
      checkTxpAlreadyExists(opts.txProposalId, function(err, txp) {
        ...
        async.series([
          ...
          function(next) {
            self._hasKeoBalance(opts, function(err, hasBalance){
              if (err) return next(err);

...
WalletService.prototype._getKeokenExplorer = function() {
  ...
  //TODO JUST CREATE A NEW INSTANCE EVERY TIME
...
WalletService.prototype._hasKeoBalance = function(opts, cb) {
  ...
  bc.getAssetsByAddress(opts.toAddress, function( error, body ){
    if(error != null)
...
WalletService.prototype._decodeKeokenScript = function (output) {
  
  var keo_prefix = "6a0400004b50"; // op_return + keoken prefix
  var version = "100000"; // 10hex = 16 bytes lenght + version (0)
  /* 
      "0001" // simple send (1)
 ...
 var send_token = "0001"; // simple send (1)
 ...
 if (keo_prefix == script.substring(0, 12) && version == script.substring(12, 18)) {
    ...
    if (type == send_token) {
      ...
      /*if (keoken_id != asset_id || output.keoken_amount != amount) {
        var error;
        error.code = 'BADREQUEST';
        error.message = 'Malformed keoken message';
        return error;
      }*/
    }
  }
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  for (var i = 0; i < opts.outputs.length; i++) {
    ...
    if (!is_script){
      ...
      var keok = this._decodeKeokenScript(output);
      if (keok != null)
        return error;
    }
        ...
        async.series([
          ...
          function(next) {
            self._hasKeoBalance(opts, function(err, hasBalance ){
              if (err) return next(err);
              if (!hasBalance) return next(Errors.TX_CANNOT_CREATE);
              next();
            });
          },
          function(next) {

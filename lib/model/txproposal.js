...
TxProposal.create = function(opts) {
  ...
  // When sending keoken transactions, shuffling the outputs may result in sending the tokens to yourself insted of the destinty address.
  // Output shuffle is dissable, so the outputs stays in the same order that the wallet-client send them to the BWS.
...
TxProposal.prototype._buildTx = function() {
  ...
  switch (self.addressType) {
    case Constants.SCRIPT_TYPES.P2SH:
      _.forEach(self.inputs, function(i) {
  ...
  _.forEach(self.outputs, function(o) {
...
TxProposal.prototype._addSignaturesToBitcoreTx = function(tx, signatures, xpub) {
  ...
  _.forEach(signatures, function(signatureHex) {

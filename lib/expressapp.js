...
ExpressApp.prototype.start = function(opts, cb) {
  ...
  // Keoken balance
  router.get('/v1/keobalance/:identifier', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      server.getKeoBalance({ id : req.params['identifier']}, function(err, balance) {
        if (err) return returnError(err, res, req);
  ...
  // Keoken assets (full list)
  router.get('/v1/keoassets', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      server.getKeoAssets(function(err, assets) {
  ...
  // Associate asset to a wallet
  router.put('/v2/wallets/:wallet_id/assets/:asset_id', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      server.storeAssetBinding(req.params['wallet_id'], req.params['asset_id'], function(err) {
        if (err) return returnError(err, res, req);
        res.json({
          success: true
        });
  ...
  // Get assets associated to a specific wallet
  router.get('/v2/wallets/:wallet_id/assets', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      server.fetchWalletAssets(req.params['wallet_id'], function(err, assets) {
        if(err) returnError(err, res, req);
        res.json(assets);
      });
    });
  ...
  router.get('/v1/feelevels/', estimateFeeLimiter, function(req, res) {
    ...
    server.getFeeLevels(opts, function(err, feeLevels) {
      if (err) return returnError(err, res, req);
      _.forEach(feeLevels, function(feeLevel) {
  ...
  router.get('/v1/txhistory/', function(req, res) {
    getServerWithAuth(req, res, function(server) {
      ...
      if (req.query.limit) opts.limit = +req.query.limit;
      if (req.query.includeExtendedInfo == '1') opts.includeExtendedInfo = true;
      if (req.query.filterByAssetId) opts.filterByAssetId = req.query.filterByAssetId;

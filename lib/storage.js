...
//                       ^to     <=  ^from
//                       ^fwdIndex  =>  ^end
Storage.prototype.getTxHistoryCache = function(walletId, from, to, assetId, cb) {
  var self = this;
  $.checkArgument(from >= 0);
  $.checkArgument(from <= to);
  $.checkArgument(assetId > 0);
...
Storage.prototype.softResetTxHistoryCache = function(walletId, assetId, cb) {

  var clearingCriterion = {
    walletId: walletId,
    type: 'historyCacheStatus',
    key: null
  }
  if (assetId) {
    clearingCriterion.assetId = assetId;
  }
...
Storage.prototype.clearTxHistoryCache = function(walletId, assetId, cb) {
  $.checkArgument(assetId > 0);
  var self = this;
...
Storage.prototype.storeTxHistoryCache = function(walletId, totalItems, firstPosition, items, assetId, cb) {
...
Storage.prototype.storeAssetBinding = function(walletId, assetId, cb) {
  this.db.collection(collections.WALLETS).update({
    id: walletId
  }, {
    "$addToSet" :
...
Storage.prototype.fetchWalletAssets = function(walletId, cb) {
  var self = this;

  self.db.collection(collections.WALLETS).findOne({
    id: walletId
  }, function(err, result) {
    if (err) return cb(err);
    if (!result) return cb();
    return cb(null, result.assets);
  });
}
